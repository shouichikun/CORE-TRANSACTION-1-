...
if (!function_exists('logActivity')) {
    function logActivity($userId, $action, $table, $recordId, $description = '') {
        global $conn;
        $sql = "INSERT INTO activity_logs (user_id, action, table_name, record_id, description, created_at) 
                VALUES (?, ?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
            return false;
        }
        $stmt->bind_param("issis", $userId, $action, $table, $recordId, $description);
        $result = $stmt->execute();
        $stmt->close();
        return $result;
    }
...
// Build query conditions - include ALL jobs including pending_review from clients
$conditions = [];
$params = [];
$types = "";

// For HR, show all jobs including client requests
// No user restriction - HR sees all jobs
...
// Get jobs with client industry - include ALL jobs including client requests
...
// =============================================
// Handle POST for update/delete/approve/reject
// =============================================
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve_job' && $jobId > 0) {
        ...
        $sql = "UPDATE job_orders SET 
                status = 'open',
                request_notes = CONCAT(COALESCE(request_notes, ''), '\n\n--- HR APPROVAL ---\n', ?),
                updated_at = NOW()
                WHERE id = ? AND status = 'pending_review'";
        
        $result = updateRecord($sql, [$feedback, $jobId], "si");
    ...
    if ($action === 'delete_job' && $jobId > 0) {
        $job = getRecord("SELECT title FROM job_orders WHERE id = ?", [$jobId], "i");
        if ($job) {
            $sql = "DELETE FROM job_orders WHERE id = ?";
            $result = deleteRecord($sql, [$jobId], "i");
            if ($result) {
...
if (isset($_GET['ajax'])) {
    ...
    if ($ajaxAction === 'edit' && $jobId > 0) {
        ...
        if ($job) {
            // Get client info for the job
            $client = getRecord("SELECT company_name FROM clients WHERE id = ?", [$job['client_id']], "i");
            $job['company_name'] = $client['company_name'] ?? 'Unknown';
            
            // Parse skills_required JSON if it's JSON
            $skillsRequired = $job['skills_required'] ?? '';
            $parsedSkills = json_decode($skillsRequired, true);
            
            if (is_array($parsedSkills) && isset($parsedSkills['skills'])) {
                // It's JSON from client - extract skills as comma-separated string for the form
                $job['skills_required_display'] = implode(', ', $parsedSkills['skills'] ?? []);
                // Also store the full parsed data for reference
                $job['skills_data'] = $parsedSkills;
            } else {
                // It's plain text (HR posted job)
...
<aside class="dashboard-sidebar" id="appSidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="jobs.php" class="sidebar-main-link <?php echo in_array(basename($_SERVER['PHP_SELF']), ['jobs.php', 'job_view.php', 'post_job.php']) ? 'active' : ''; ?>">
            ...
            <?php 
            $pendingReviewCount = getRecord("SELECT COUNT(*) as count FROM job_orders WHERE status = 'pending_review'", [], "")['count'] ?? 0;
