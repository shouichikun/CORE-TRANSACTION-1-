    ...
    if ($enrollUserId > 0) {
        $user = getRecord("SELECT id, first_name, last_name, email, role FROM users WHERE id = ?", [$enrollUserId], "i");
        if ($user) {
    ...
    $hasFace = false;
    if ($enrollUserId > 0) {
        $existing = getRecord("SELECT id FROM face_scans WHERE user_id = ?", [$enrollUserId], "i");
