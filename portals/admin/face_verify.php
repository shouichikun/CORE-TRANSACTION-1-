...
if ($verifyUserId > 0) {
    $user = getRecord("SELECT id, first_name, last_name, email, role FROM users WHERE id = ?", [$verifyUserId], "i");
    if ($user) {
...
$faceData = null;
if ($verifyUserId > 0) {
    $faceData = getRecord("SELECT id, face_encoding, liveness_score, created_at FROM face_scans WHERE user_id = ?", [$verifyUserId], "i");
