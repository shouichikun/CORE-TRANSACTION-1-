<?php
// portals/admin/face_verify.php - Face Verification for Users
session_start();
...
// Get user to verify
$verifyUserId = isset($_GET['user_id']) ? intval($_GET['user_id']) : 0;
$user = null;
$userName = '';

// ✅ FIXED: PostgreSQL uses $1 placeholder, removed type string
if ($verifyUserId > 0) {
    $user = getRecord("SELECT id, first_name, last_name, email, role FROM users WHERE id = $1", [$verifyUserId]);
    if ($user) {
        $userName = $user['first_name'] . ' ' . $user['last_name'];
    }
}

// ✅ FIXED: Check if user has a face enrolled - PostgreSQL uses $1 placeholder
$hasFace = false;
$faceData = null;
if ($verifyUserId > 0) {
    $faceData = getRecord("SELECT id, face_encoding, liveness_score, created_at FROM face_scans WHERE user_id = $1", [$verifyUserId]);
    $hasFace = !empty($faceData);
}
...
<head>
    ...
    <title>Face Verification - ISMERS</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=Public+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&display=swap" rel="stylesheet">
    <style>
        /* ==========================================================================
           MATERIAL 3 DESIGN SYSTEM - FACE VERIFICATION
           ========================================================================== */
        :root {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* =============================================
           SIDEBAR - FIXED (same as face_enroll.php)
        ============================================= */
        .dashboard-sidebar {
            position: fixed;
            top: 0;
        ...
        .verify-card .card-header h3 .material-symbols-outlined {
            font-size: 1.25rem;
            color: var(--primary);
        }
        ...
        .face-controls .btn-danger {
            background: var(--error-color);
            color: white;
        }
        .face-controls .btn-danger:hover:not(:disabled) {
            transform: translateY(-2px);
        ...
        .result-badge.failed { background: #fecaca; color: #dc2626; }
        ...
        @media (max-width: 480px) {
            ...
            .verify-card .card-header { padding: 1rem 1.25rem; }
            .verify-card .card-body { padding: 1rem 1.25rem; }
...
<body>
    ...
    <div class="main-wrapper" id="mainWrapper">
        <header class="top-header">
            <div class="top-header-left">
                ...
                <span style="font-weight:600; font-size:0.875rem;">Face Verification</span>
            </div>
            ...
            <div class="container">
                ...
                <div class="breadcrumb-bar">
                    <div class="breadcrumb-view">
                        ...
                        <span style="font-weight:400; color:var(--text-on-surface-variant);">
                            <?php echo $verifyUserId > 0 && $user ? htmlspecialchars($userName) : 'Select a user'; ?>
                        </span>
                    </div>
                ...
                <div class="page-header">
                    <div>
                        ...
                        <p>Verify a user's identity using facial recognition</p>
                    </div>
                </div>

                <?php if (!$verifyUserId || !$user): ?>
                    <!-- No user selected -->
                    <div class="verify-card">
                        <div class="card-body" style="text-align:center; padding:3rem 1.5rem;">
                            <span class="material-symbols-outlined" style="font-size:4rem; color:var(--slate-300); display:block; margin-bottom:0.75rem;">verified</span>
                            <h3 style="font-size:1.125rem; font-weight:700; margin-bottom:0.25rem;">No User Selected</h3>
                            <p style="color:var(--text-on-surface-variant); font-size:0.875rem;">Please go to Biometric Settings and click "Verify" on a user.</p>
                ...
                <?php elseif (!$hasFace): ?>
                    ...
                    <div class="verify-card">
                        <div class="card-body" style="text-align:center; padding:3rem 1.5rem;">
                            ...
                            <h3 style="font-size:1.125rem; font-weight:700; margin-bottom:0.25rem;">No Face Enrolled</h3>
                            <p style="color:var(--text-on-surface-variant); font-size:0.875rem;"><?php echo htmlspecialchars($userName); ?> has not enrolled a face yet.</p>
                            <a href="face_enroll.php?user_id=<?php echo $verifyUserId; ?>" class="btn btn-primary" style="margin-top:1rem; display:inline-flex;">
                        ...
                        <div>
                            <div class="verify-card">
                                <div class="card-header">
                                    ...
                                    <span class="status-badge enrolled" style="background:#d1fae5; color:#059669; padding:0.25rem 0.75rem; border-radius:var(--radius-full); font-size:0.75rem; font-weight:600;">✅ Enrolled</span>
                                </div>
                                <div class="card-body">
                                    ...
                                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem; margin-top:1rem;">
                                        <div style="padding:0.75rem; background:var(--bg-surface-low); border-radius:var(--radius); text-align:center;">
                                            <div style="font-size:0.65rem; color:var(--text-on-surface-variant);">Enrolled Since</div>
                                            <div style="font-weight:600; font-size:0.875rem;"><?php echo date('M d, Y', strtotime($faceData['created_at'])); ?></div>
                                        </div>
                                        <div style="padding:0.75rem; background:var(--bg-surface-low); border-radius:var(--radius); text-align:center;">
                                            <div style="font-size:0.65rem; color:var(--text-on-surface-variant);">Liveness Score</div>
                                            <div style="font-weight:600; font-size:0.875rem;"><?php echo $faceData['liveness_score'] ? number_format($faceData['liveness_score'] * 100, 1) . '%' : '—'; ?></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="verify-card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="face-controls">
                                        <button class="btn btn-primary" id="verifyBtn" onclick="verifyFace()" disabled>
                                            <span class="material-symbols-outlined">verified</span>
                                            Verify Face
                                        </button>
                                    ...
                                    <!-- Result -->
                                    <div id="resultContainer" style="display:none; margin-top:1rem; padding:1rem; border-radius:var(--radius); text-align:center;">
    ...
    <script>
        ...
        <?php if ($verifyUserId && $user && $hasFace && $faceData): ?>
        const VERIFY_USER_ID = <?php echo $verifyUserId; ?>;
        const VERIFY_USER_NAME = '<?php echo htmlspecialchars($userName); ?>';
        const STORED_DESCRIPTOR = <?php echo $faceData['face_encoding']; ?>;
        <?php else: ?>
        const VERIFY_USER_ID = 0;
        const VERIFY_USER_NAME = '';
        const STORED_DESCRIPTOR = null;
        <?php endif; ?>
        ...
        function showResult(success, message, score = null) {
            ...
            if (success) {
                ...
                content.innerHTML = `
                    <div style="display:flex; align-items:center; justify-content:center; gap:0.75rem;">
                        <span class="material-symbols-outlined" style="color:#059669; font-size:2rem;">verified</span>
                        <div>
                            <div style="font-weight:700; color:#065f46; font-size:1.125rem;">✅ Verification Successful</div>
                            <div style="color:#047857; font-size:0.875rem;">${message}</div>
                            ${score !== null ? `<div style="color:#047857; font-size:0.75rem; margin-top:0.25rem;">Match Score: ${score}%</div>` : ''}
                        </div>
                    </div>
                `;
            } else {
                container.style.background = '#fecaca';
                content.innerHTML = `
                    <div style="display:flex; align-items:center; justify-content:center; gap:0.75rem;">
                        <span class="material-symbols-outlined" style="color:#dc2626; font-size:2rem;">error</span>
                        <div>
                            <div style="font-weight:700; color:#991b1b; font-size:1.125rem;">❌ Verification Failed</div>
                            <div style="color:#b91c1c; font-size:0.875rem;">${message}</div>
                            ${score !== null ? `<div style="color:#b91c1c; font-size:0.75rem; margin-top:0.25rem;">Match Score: ${score}%</div>` : ''}
                        </div>
                    </div>
                `;
            }
        ...
        async function verifyFace() {
            if (!isInitialized || isVerified || !STORED_DESCRIPTOR) {
            ...
            btn.disabled = true;
            btn.innerHTML = '<span class="ai-dots-loading-sm"><div class="dot"></div><div class="dot"></div><div class="dot"></div></span> Verifying...';

            const wrapper = document.getElementById('faceWrapper');
            if (wrapper) wrapper.classList.add('scanning');

            updateStatus('Verifying face...', 'loading');
            document.getElementById('faceStatus').textContent = 'Verifying...';
            ...
            try {
                ...
                if (!detection) {
                    ...
                    showResult(false, 'No face detected. Please position your face clearly.');
                    btn.disabled = false;
                    btn.innerHTML = '<span class="material-symbols-outlined">verified</span> Verify Face';
                ...
                const storedDesc = new Float32Array(STORED_DESCRIPTOR);
                ...
                updateStatus(isMatch ? '✅ Verified!' : '❌ No match', isMatch ? 'success' : 'failed');
                document.getElementById('faceStatus').textContent = isMatch ? `✅ Verified! ${matchScore}%` : `❌ No match (${matchScore}%)`;
                document.getElementById('faceStatus').className = `face-status show ${isMatch ? 'success' : 'failed'}`;

                if (wrapper) {
                    wrapper.classList.remove('scanning');
                    wrapper.classList.add(isMatch ? 'success' : 'failed');
                }

                // Show result
                if (isMatch) {
                    showResult(true, `Face verified for ${VERIFY_USER_NAME}`, matchScore);
                    showToast('✅ Face verified successfully!', 'success');
                    
                    // Log verification
                    await logVerification(true, matchScore);
                } else {
                    showResult(false, `Face does not match ${VERIFY_USER_NAME}`, matchScore);
                    showToast('❌ Face verification failed', 'error');
                    
                    // Log verification
                    await logVerification(false, matchScore);
                }

            } catch (error) {
                console.error('Verification error:', error);
                showResult(false, 'Error during verification: ' + error.message);
        ...
        // LOG VERIFICATION
        // =============================================
        async function logVerification(success, score) {
            try {
                const response = await fetch('/CT1/api/face/log_verification.php', {
                    method: 'POST',
        ...
        async function initFaceAuth() {
            if (!VERIFY_USER_ID || !STORED_DESCRIPTOR) {
                updateStatus('No stored face data available', 'idle');
                return;
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            if (VERIFY_USER_ID > 0 && STORED_DESCRIPTOR) {
                initFaceAuth();
            } else {
                updateStatus('No user selected or no face enrolled', 'idle');
            }
        });
