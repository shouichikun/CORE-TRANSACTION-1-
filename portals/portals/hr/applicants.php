...
function updateAIMatchScore($applicationId) {
    ...
    } elseif (isset($matchResult['match_score'])) {
        $score = (int)$matchResult['match_score'];
    }
    
    $level = $matchResult['level'] ?? 'N/A';
    $recommendation = $matchResult['recommendation'] ?? 'No recommendation available';
    $matchedSkills = $matchResult['matched_skills'] ?? $matchResult['strengths'] ?? [];
    $missingSkills = $matchResult['missing_skills'] ?? $matchResult['gaps'] ?? [];
    ...
    $matchDetails = json_encode([
        ...
        'missing_skills' => $missingSkills,
        'breakdown' => $breakdown,
        'skills_score' => $details['skills_match'] ?? $breakdown['skills'] ?? 0,
        'experience_score' => $details['experience_years'] ?? $breakdown['experience'] ?? 0,
        'education_score' => $details['education_level'] ?? $breakdown['education'] ?? 'Not specified',
        'cover_letter_score' => $details['cover_letter_score'] ?? $breakdown['cover_letter'] ?? 0,
