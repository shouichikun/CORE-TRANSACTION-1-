    ...
    if ($file['error'] === 0) {
        if (!in_array($file['type'], $allowedTypes)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filepath)) {
                // ✅ FIXED: Update database - PostgreSQL uses $1, $2 placeholders
                $relativePath = 'uploads/profile_pictures/' . $filename;
                $updateSql = "UPDATE applicants SET profile_picture = $1 WHERE user_id = $2";
                $updated = updateRecord($updateSql, [$relativePath, $userId]);
