...
Important Rules:
1. Score should be 0-100 based on how well the applicant matches the job
2. Consider skills match, experience, education, and cover letter quality
3. Be realistic - if applicant has most skills but lacks some, score should be high but not perfect

Return ONLY a valid JSON object with these exact keys:
{
    \"score\": 75,
    \"level\": \"Good\",
    \"recommendation\": \"Candidate is a good match for this role. Consider for interview.\",
    ...
    if ($response && !isset($response['error'])) {
        ...
        return [
            ...
            'recommendation' => $response['recommendation'] ?? 'Candidate is a good match.',
            'matched_skills' => $matched,
            'missing_skills' => $missing,
            'breakdown' => $breakdown,
            'details' => [
                'skills_match' => $breakdown['skills'] ?? $details['skills_match'] ?? 0,
                'experience_years' => $details['experience_years'] ?? 0,
                'education_level' => $details['education_level'] ?? 'Not specified',
                'cover_letter_score' => $breakdown['cover_letter'] ?? $details['cover_letter_score'] ?? 0,
...
Return ONLY valid JSON.";
   ...
   private function mockMatchScore($jobData, $applicantData) {
    ...
    if (count($matchedSkills) > 0) {
        $strengths[] = 'Has ' . count($matchedSkills) . ' of ' . $totalJobSkills . ' required skills';
    }
    if (count($matchedSkills) >= $totalJobSkills * 0.7) {
        $strengths[] = 'Strong skills match for this role';
    }

    if (count($missingSkills) > 0) {
        $gaps[] = 'Missing ' . count($missingSkills) . ' required skills: ' . implode(', ', array_slice($missingSkills, 0, 3));
    }
    if (count($missingSkills) >= $totalJobSkills * 0.5) {
        $gaps[] = 'Significant skill gaps to address';
    }

    if (empty($strengths)) {
        $strengths = ['Good fit for the role', 'Relevant background'];
    }
    if (empty($gaps)) {
    ...
    if ($score >= 80) {
        $recommendation = 'Highly recommended - Strong match for this position. Proceed to interview.';
    } elseif ($score >= 60) {
        $recommendation = 'Recommended - Good fit with some areas to explore during interview.';
    } elseif ($score >= 40) {
    ...
    $breakdown = [
        ...
        'cover_letter' => max(0, $score - rand(0, 40))
    ];
    ...
    return [
        ...
        'details' => [
            ...
            'education_level' => 'Bachelor\'s Degree',
            'cover_letter_score' => $breakdown['cover_letter'],
